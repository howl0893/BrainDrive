...
class CustomDropdown extends React.Component<CustomDropdownProps, CustomDropdownState> {
  ...
  componentDidUpdate(prevProps: CustomDropdownProps, prevState: CustomDropdownState) {
    ...
    // If dropdown is open and search term changed, re-render options
    if (this.state.isOpen && prevState.searchTerm !== this.state.searchTerm) {
    ...
    try {
      ...
      const filteredOptions = searchTerm.trim() 
        ? options.filter(option => 
            option.primaryText.toLowerCase().includes(searchTerm.toLowerCase()) ||
            option.secondaryText.toLowerCase().includes(searchTerm.toLowerCase())
          )
        : options;
      ...
      if (highlightedIndex >= 0 && filteredOptions[highlightedIndex]) {
        optionsList.setAttribute('aria-activedescendant', `option-${filteredOptions[highlightedIndex].id}`);
      }
  ...
  handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    const { isOpen, highlightedIndex, searchTerm } = this.state;
    
    if (disabled) return;

    // Get filtered options for navigation
    const filteredOptions = searchTerm.trim() 
      ? options.filter(option => 
          option.primaryText.toLowerCase().includes(searchTerm.toLowerCase()) ||
          option.secondaryText.toLowerCase().includes(searchTerm.toLowerCase())
    ...
    if (isOpen && e.key === 'Backspace') {
      ...
      this.setState(prevState => ({
        searchTerm: prevState.searchTerm.slice(0, -1),
        highlightedIndex: 0
      }));
      return;
    }
    ...
    switch (e.key) {
      case 'ArrowDown':
        ...
        if (!isOpen) {
          ...
          this.setState(prevState => ({
            highlightedIndex: prevState.highlightedIndex < filteredOptions.length - 1 
      ...
      case ' ':
        ...
        if (isOpen && highlightedIndex >= 0 && filteredOptions[highlightedIndex]) {
          this.props.onChange(filteredOptions[highlightedIndex].id);
  ...
  toggleDropdown = () => {
    ...
    if (!disabled) {
      this.setState(prevState => {
        const newState: Partial<CustomDropdownState> = { 
          isOpen: !prevState.isOpen,
          searchTerm: '' // Clear search when toggling
        };
